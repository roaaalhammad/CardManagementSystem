...
const FINAL_STATUSES = ["تم التسليم", "مرفوض من التواصل الداخلي"];
...
export default function RequestsList() {
    ...
    try {
      ...
      setRequests(
        data.map((r) => ({
          ...
          readyForDelivery: r.readyForDelivery && !r.isDelivered,
  ...
  const filteredRequests = useMemo(() => {
    if (activeTab === "الطلبات الجديدة") {
      return requests.filter((r) => r.status === "قيد مراجعة التواصل الداخلي");
    }
    if (activeTab === "الطلبات تحت الإجراء") {
      return requests.filter((r) => r.status === "بانتظار التسليم");
    }
      ...
      {!loading && !error && (
        <>
          <DataTable
            ...
            emptyMessage={
              activeTab === "الطلبات الجديدة"
                ? "لا يوجد طلبات جديدة"
                : activeTab === "الطلبات تحت الإجراء"
                ? "لا يوجد طلبات تحت الإجراء"
                : "لا يوجد طلبات سابقة"
            }
          />
